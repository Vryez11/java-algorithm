package src.baekjoon.doit.prefixSum;
...
public class Problem4 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= M; i++) {
            for (int j = 1; j <= M; j++) {
                if (i == 1) S[i][j] = S[i][j - 1] + sc.nextLong();
                else if (j == 1) S[i][j] = S[i - 1][j] + sc.nextLong();
                else S[i][j] = S[i - 1][j] + S[i][j - 1] - S[i - 1][j - 1] + sc.nextLong();
            }
        }
