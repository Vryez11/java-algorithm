----------------------------
# 프로그래머스 / 백준
----------------------------

+ 프로그래머스 Lv1
1. 약수의 합
2. 두 정수 사이의 합
3. 문자열 내 p와 y의 개수=
4. 정수 내림차순으로 배치하기
5. 정수 제곱근 판별
6. 하샤드 수
7. 나누어 떨어지는 숫자 배열
8. 서울에서 김서방 찾기
9. 콜라츠 추측
10. 음양 더하기
11. 없는 숫자 더하기
12. 제일 작은 수 제거하기
13. 핸드폰 번호 가리기
14. 내적
15. 가운데 글자 가져오기
16. 수바수박수박수박수박수?
17. 약수의 개수와 덧셈
18. 문자열 내림차순으로 배치하기
19. 부족한 금액 계산하기
20. 문자열 다루기 기본
21. 행렬의 덧셈
22. 직사각형 별찍기
23. 같은 숫자는 싫어
24. 최대 공약수와 최소공배수
25. 큭기가 작은 부분 문자열
26. 이상한 문자 만들기
27. 예산
28. 3진법 뒤집기
29. 삼총사
30. 최소직사각형
31. 시저 암호
32. 가장 가까운 같은 글자
33. 두 개 뽑아서 더하기
34. 숫자 문자열과 영단어
35. 푸드 파이트 대회
36. 문자열 내 마음대로 정렬하기
37. K번째수
38. 콜라 문제
39. 명예의 전당(!)
40. [1차] 비밀지도
41. 카드 뭉치
42. 추억 점수
43. 폰켓몬
44. 기사단원의 무기
45. 2016년
46. 모의고사
47. 과일 장수
48. 소수 찾기
49. 소수 만들기
50. 덧칠하기
51. 실패율
52. 옹알이(2)
53. [PCCE 기출문제] 9번 / 지폐 접기
54. [1차] 다트 게임
55. 로또의 최고 순위와 최저 순위
56. 문자열 나누기
57. 대충 만든 자판
58. [1차] 다트 게임
59. 완주하지 못한 선수
60. [PCCE기출문제] 9번 / 이웃한 칸
61. 체육복
62. 숫자 짝꿍
63. 햄버거 만들기
64. 가장 많이 받은 선물 (2024 KAKAO WINTER INTERNSHIP), 1Ref 데이터 전처리 부분 수정
    + 문제 고찰 : https://velog.io/@vryez1_/JAVA-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EC%8A%A4-1-%EA%B3%A0%EC%B0%B0   
65. PCCP 기출문제 1번 / 붕대 감기 (PCCP 기출문제), 1Ref 중복 코드 및 필요없는 조건 수정
    + 문제 고찰 : https://velog.io/@vryez1_/JAVA-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EC%8A%A4-2-%EB%B2%88%EC%A7%B8-%EA%B3%A0%EC%B0%B0
66. 옹알이 1 (코딩테스트 입문)
67. 달리기 경주 (연습문제)
68. 바탕화면 정리 (연습문제)
69. 신고 결과 받기 (2022 KAKAO BLIND RECRUITMENT)
70. 공원 산책 (연습문제)
    + 문제 고찰 : https://velog.io/@vryez1_/Java-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EC%8A%A4-3-%EA%B3%A0%EC%B0%B0
71. 개인정보 수집 유효기한 (2023 KAKAO BLIND RECRUITMENT)
72. 정수를 나선형으로 배치하기 (코딩 기초 트레이닝)
73. 신규 아이디 추천 (2021 KAKAO BLIND RECRUIMENT)
74. 성격 유형 검사 (2022 KAKAO TECH INTERSHIP)
75. [PCCP 기출문제] 10번 / 데이터 분석 
76. 크레인 인형 뽑기 (2019 카카오 개발자 겨울 인턴쉽)
77. 키패드 누르기 (2020 카카오 인턴쉽)
    + 문제 고찰 : https://velog.io/@vryez1_/Java-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EC%8A%A4-%EB%AC%B8%EC%A0%9C-4-%EA%B3%A0%EC%B0%B0
78. 평행 (코딩테스트 입문)
79. [PCCP 기출문제] 10번 / 공원
80. [PCCP 기출문제] 1번 / 동영상 재생기

+ 프로그래머스 Lv2
1. 최댓값과 최솟값
2. 최솟값 만들기
3. JadenCase 문자열 만들기
4. 올바른 괄호
5. 이진 변환 반복하기
6. 숫자의 표현
7. 다음 큰 숫자
8. 피보나치 수
9. 짝지어 제거하기
10. 카펫
11. 구명보트
12. 점프와 순간 이동
13. 멀리 뛰기
14. N개의 최소공배수
15. 영어 끝말잇기
16. 귤 고르기
17. 예상 대진표
18. 연석 부분 수열 합의 개수
19. 괄호 회전하기
20. 할인 행사
21. n^2 배열 자르기
22. 행렬의 곱셈
23. 의상
24. H-Index
25. [1차] 캐시
26. 기능개발
27. 피로도
28. 튜플
29. 전화번호 목록
30. 프로세스
31. [1차] 뉴스 클러스터링
32. 타켓 넘버
33. 롤케이크 자르기
34. 게임 맵 최단거리
35. 방문 길이
36. 모음사전
37. k진수에서 소수 개수 구하기
38. [3차] 압축
39. [3차] n진수 게임
40. 뒤에 있는 큰 수 찾기
41. 더 멥게
42. 주식가격
43. 땅따먹기
44. 스킬트리
45. 택배상자
46. 주차 요금 계산
47. 숫자 변환하기
48. [3차] 파일명 정렬
49. 오픈채팅방
50. 2 X n 타일링
51. [1차] 프렌즈4블록
52. 2개 이하로 다른 비트
53. 가장 큰 수
54. 쿼드압축 후 개수 세기
55. 두 큐 합 같게 만들기
56. 소수 찾기
57. 다리를 지나는 트럭
58. 삼각 달팽이
59. 큰 수 만들기
60. 연속된 부분 수열의 합
61. 마법의 엘리베이터
62. 시소 짝꿍
63. 전력망을 둘로 나누기
64. 호텔 대실
65. 124 나라의 숫자
66. 서버 증설 횟수
67. 숫자 카드 나누기
68. 미로 탈출
69. 메뉴 리뉴얼
70. 배달
71. [3차] 방금그곡
72. 무인도 여행
73. 줄 서는 방법
74. 괄호 변환
75. [카카오 인턴] 수식 최대화
76. 리코쳇 로봇
77. 행렬 테두리 회전하기
78. 테이블 해시 함수
79. 디펜스 게임
80. 거리두기 확인하기
81. 가장 큰 정사각형 찾기
82. 하노이의 탑

-----------------------------

+ 백준
1. 단계별로 풀어보기 1단계 - (입출력과 사칙연산)
   + (2557번, 1000번, 1001번, 10998번, 1008번, 10869번, 10926번, 18108번, 10430번, 2588번, 11382번, 10171번, 10172번)
2. 단게별로 풀어보기 2단계 - (조건문)
   + (1330번, 9498번, 2753번, 14681번, 2884번, 2525번, 2480번)
3. 단계별로 풀어보기 3단계 - (반복문)
   + (2739번 ,10950번, 8393번, 25304번, 25314번, 15552번, 11021번, 11022번, 2438번, 2439번, 10952번, 10951번)
4. 단계별로 풀어보기 4단계 - (1차원 배열)
   + (10807번, 10871번, 10818번, 2562번, 10810번, 10813번)
5. 단계별로 풀어보기 5단계 - (문자열)
   + (27866번, 2743번, 9086번, 11654번, 11720번, 10809번, 2675번, 1152번, 2908번, 5622번, 11718번)
6. 단계별로 풀어보기 6단계 - (심화1)
   + (25083번, 3003번, 2444번, 10988번, 1157번, 2941번, 1316번, 25206번)
7. 단계별로 풀어보기 7단계 - (2차원 배열)
   + (2738번, 2566번, 10798번, 2563번)
8. 단계별로 풀어보기 8단계 - (일반 수학 1)
    + (2745번, 11005번, 2720번, 2903번, 2292번, 1193번, 2869번)
9. 단계별로 풀어보기 9단계 - (약수, 배수와 소수)
    + (5086번, 2501번, 9506번, 1978번, 2581번)
10. 단계별로 풀어보기 10단계 - (기하: 직사각형과 삼각형)
    + (27323번, 1085번, 3009번, 15894번, 9063번, 10101번, 5073번, 14215번)
11. 단계별로 풀어보기 11단계 - (시간 복잡도)
    + (24262번, 24263번, 24264번, 24265번, 24266번, 24267번, 24313번)
12. 단계별로 풀어보기 12단계 - (브루트 포스)
    + (2798번, 2231번, 19532번, 1018번, 1436번、 2839번)
13. 단계별로 풀어보기 13단계 - (정렬)
    + (2750번, 2587번, 25305번, 2751번, 10989번, 1427번, 11650번, 11651번, 1181번, 10814번, 18870번)
14. 단계별로 풀어보기 14단계 - (집합과 맵)
    + (10815번, 14425번, 7785번, 1620번, 10816번, 1764번, 1269번, 11478번)
15. 단계별로 풀어보기 15단계 - (약수, 배수와 소수 2)
    + (1934번, 13241번, 1735번, 2485번, 4134번, 1929번, 4948번, 17103번, 13909번)
16. 단계별로 풀어보기 16단계 - (스택, 큐, 덱)
    + (28278번, 10773번, 9012번, 4949번, 12789번, 18258번, 2164번, 11866번, 28279번, 2346번, 24511번)
   
+ SW 마에스트로 대비 문제
    + 1463번, 2579번, 9095번, 11726번, 1149번, 1932번

+ Do it! 알고리즘 코딩 테스트
1. 시간 복잡도
   + (2750번)
2. 자료 구조
    + 배열과 리스트
        + (11720번, 1546번)
    + 구간 합
        + (11659번, 11660번, 10986번)
    + 투 포인터
        + (2018번, 1940번, 1253번)
    + 슬라이딩 윈도우
        + (12891번, 11003번)
    + 스택과 큐
        + (1874번, 17298번, 2164번, 11286번)
3. 정렬
    + 버블 정령
        + (2750번, 1377번)
    + 선택 정령
        + (1427번)
    + 삽입 정렬
        + (11399번)
    + 퀵 정렬
        + (11004번)
    + 병합 정렬
        + (2751번, 1517번)
    + 기수 정렬
        + (10989번)
4. 탐색
    + 깊이 우선 탐색
        + (11724번, 2023, 13023번)
    + 너비 우선 탐색
        + (1260번, 2178번, 1167번)
    + 이진 탐색
        + (1920번, 2343번, 1300번)
5. 그리디
    + 그리디 알고리즘
        + (11047번, 1715번, 1744번, 1931번, 1541번)
6. 정수론
    + 소수 구하기
        + (1929번, 1456번, 1747번, 1016번)
    + 오일러피
        + (11689번)
    + 유클리드 호제법
        + (1934번, 1850번, 1033번)
    + 확장 유클리드 호제법
        + (21568번)
7. 그래프
    + 그래프의 표현
        + (18352번, 1325번, 1707번, 2251번)
    + 유니온 파인드
        + (1717번, 1976번, 1043번)
    + 위상 정렬
        + (2252번, 1516번, 1948번)
    + 다익스트라
        + (1753번, 1916번, 1854번)
    + 벨만-포드
        + (11657번, 1219번)
    + 플로이드-워셜
        + (11404번, 11403번, 1389번)
    + 최소 신장 트리
        + (1197번, 17472번, 1414번)
8. 트리
    + 트리 알아보기
        + (11725번, 1068번)
    + 트라이
        + (14425번)
    + 이진 트리
        + (1991번)
    + 세그먼트 트리
        + (2042번, 10868번, 11505번)
    + 최소 공통 조상 (LCA)
        + (11437번, 11438번)
9. 조합
    + (11050번, 11051번, 2775번, 1010번,13251번 , 1722번, 1256번, 1947번)
10. 동적 계획법
    + (1463번, 14501번, 2193번, 11726번, 10844번, 13398번, 9252번, 1915번, 1328번, 2342번)
